...
import {AddNivel} from "./add-nivel";
import DeleteNivel from "./detete-nivel";
...
export const CollapseItems = ({ icon, items, title, onItemClick }: Props) => {
        ...
        >
          <div className="pl-12">
            {items.map((item, index) => (
              <span
                key={index}  onClick={() => onItemClick([item])}
                className="w-full flex  text-default-500 justify-between hover:text-default-900 transition-colors"
              >
                {item} 
              </span>
            ))}
